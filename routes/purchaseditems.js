...
router.get("/:user_id", async function (req, res) {
  // NOTE: front-end fetch must pass user_id
...
router.get("/shops/:shop_id", async function (req, res) {
  // NOTE: front-end fetch must pass shop_id
...
router.post("/", async (req, res) => {
  // NOTE: front-end fetch must pass purchase_id
  ...
  try {
    ...
    for (let i = 0; i < cartProducts.length; i++) {
      let sql = `
        ...
        VALUES ('${Number(cartProducts[i].quantity)}', '${Number(
        cartProducts[i].totalPoints
      )}', '${Number(purchaseId)}', '${Number(cartProducts[i].id)}', ${Number(
        cartProducts[i].shop_id
      )})
    `;
    ...
    let result = await db(
      `SELECT * FROM purchased_items WHERE purchase_id = ${Number(purchaseId)}`
    ); // purchaseId taken from req.body
