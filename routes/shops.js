...
router.put("/edit/:shopId/:userId", ensureShopOwner, async (req, res) => {
  ...
  try {
    ...
    if (user.shop_id === shop.shop_id && shop_address) {
      await db(
        `UPDATE shops SET shop_address='${shop_address}' WHERE shop_id=${shopId}`
      );
    }
    ...
    if (user.shop_id === shop.shop_id && shop_image) {
      await db(
        `UPDATE shops SET shop_image='${shop_image}' WHERE shop_id=${shopId}`
      );
    }
    ...
    if (user.shop_id === shop.shop_id && shop_email) {
      await db(
        `UPDATE shops SET shop_email='${shop_email}' WHERE shop_id=${shopId}`
      );
    }

    if (user.shop_id === shop.shop_id && donate) {
      await db(
        `UPDATE shops SET donate='${Number(donate)}' WHERE shop_id=${shopId}`
      );
    }
    ...
    if (user.shop_id === shop.shop_id && min_biz) {
      await db(
        `UPDATE shops SET min_biz='${Number(min_biz)}' WHERE shop_id=${shopId}`
      );
    }

    if (user.shop_id === shop.shop_id && wo_biz) {
      await db(
        `UPDATE shops SET wo_biz='${Number(wo_biz)}' WHERE shop_id=${shopId}`
      );
    }

    const results = await db(
      `SELECT * FROM shops WHERE shop_id = ${Number(shopId)}`
    );
